...
export class OAuthService {
  ...
  async getAuthorizeUrl(platform: string, customerId: string): Promise<string> {
    ...
    const state = encrypt(JSON.stringify({ customerId, platform, timestamp: Date.now() }));
    ...
    switch (platform.toLowerCase()) {
      ...
      case 'okta':
        ...
        if (!oktaDomain) throw new BadRequestException('Okta domain not configured on server');
  ...
  private async exchangeOktaToken(code: string, customerId: string) {
    const oktaDomain = this.configService.get('OKTA_DOMAIN');
    const response = await axios.post(`https://${oktaDomain}/oauth2/v1/token`, new URLSearchParams({
    ...
    if (!access_token) throw new BadRequestException('Failed to exchange Okta token');
  ...
  private async exchangeSlackToken(code: string, customerId: string) {
    ...
    if (!response.data.ok) throw new BadRequestException(`Slack error: ${response.data.error}`);
