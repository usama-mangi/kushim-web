...
describe('ComplianceCheckProcessor', () => {
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        {
          provide: getQueueToken(QueueName.COMPLIANCE_CHECK),
          useValue: mockQueue,
        },
        {
          provide: getQueueToken(QueueName.EVIDENCE_COLLECTION),
          useValue: mockQueue,
        },
      ],
    }).compile();
  ...
  describe('handleRunCheck', () => {
    ...
    it('should trigger remediation when evidence is FAIL', async () => {
      ...
      mockJiraService.createRemediationTicket.mockResolvedValue({
        status: 'SUCCESS',
        data: { issueKey: 'ISSUE-1', issueId: 'id1' },
      });
    ...
    it('should trigger evidence collection if missing', async () => {
      ...
      expect(mockQueue.add).toHaveBeenCalledWith(
        EvidenceCollectionJobType.COLLECT_AWS,
    ...
    it('should trigger GitHub collection if control is GITHUB', async () => {
      ...
      expect(result.success).toBe(false);
      expect(mockQueue.add).toHaveBeenCalledWith(
        EvidenceCollectionJobType.COLLECT_GITHUB,
        expect.objectContaining({
  ...
  describe('handleScheduleChecks', () => {
    ...
    it('should schedule checks for controls that need it', async () => {
      ...
      expect(result.success).toBe(true);
      expect(mockQueue.add).toHaveBeenCalledWith(
        ComplianceCheckJobType.RUN_CHECK,
        expect.any(Object),
      );
    });
  });
